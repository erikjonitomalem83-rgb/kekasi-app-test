...
export async function reserveNomorBerurutan(userId, formData, jumlah) {
  try {
    ...
    // PENTING: All-or-nothing approach
    // Jika tidak dapat semua yang diminta, anggap gagal
    if (resultData.length < jumlah) {
      console.warn(`[reserveNomorBerurutan] Partial allocation detected: ${resultData.length}/${jumlah}`);

      // Rollback: batalkan semua nomor yang sudah di-reserve
      if (resultData.length > 0) {
        console.log(`[reserveNomorBerurutan] Rolling back ${resultData.length} reserved numbers...`);

        try {
          const rollbackIds = resultData.map((n) => n.id);
          await supabase
            .from("nomor_surat")
            .update({
              status: "cancelled",
          ...
          console.log(`[reserveNomorBerurutan] Rollback completed`);
        } catch (rollbackError) {
          console.error(`[reserveNomorBerurutan] Rollback error:`, rollbackError);
        }
      }
      ...
      return {
        success: false,
        error: "SYSTEM_BUSY",
        message: `Sistem sedang digunakan user lain. Hanya tersedia ${resultData.length} dari ${jumlah} nomor yang diminta. Silakan coba lagi dalam beberapa saat.`,
...
export async function reserveNomorAcak(userId, formData, jumlah) {
  try {
    ...
    // Jika tidak dapat semua yang diminta, anggap gagal
    if (resultData.length < jumlah) {
      console.warn(`[reserveNomorAcak] Partial allocation detected: ${resultData.length}/${jumlah}`);

      // Rollback: batalkan semua nomor yang sudah di-reserve
      if (resultData.length > 0) {
        console.log(`[reserveNomorAcak] Rolling back ${resultData.length} reserved numbers...`);
        ...
        try {
          ...
          console.log(`[reserveNomorAcak] Rollback completed`);
        } catch (rollbackError) {
          console.error(`[reserveNomorAcak] Rollback error:`, rollbackError);
        }
      }
