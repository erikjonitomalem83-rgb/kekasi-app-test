...
export default function CountdownTimer({ expiredAt, onExpired }) {
  ...
  return (
    <div
      className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border-2 font-mono text-sm font-semibold transition-all duration-200 ${getColorClass()} ${
        isFlashing ? "scale-110 shadow-lg" : ""
      }`}>
      {isExpired ? (
        <>
