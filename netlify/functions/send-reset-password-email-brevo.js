...
exports.handler = async (event, context) => {
  console.log("[NETLIFY_FUNCTION_BREVO] Request received");
  ...
  try {
    ...
    console.log("[NETLIFY_FUNCTION_BREVO] Request body:", { email, nama_lengkap });

    if (!email || !resetLink) {
      console.error("[NETLIFY_FUNCTION_BREVO] Missing required fields");
      return {
        statusCode: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    if (!BREVO_API_KEY) {
      console.error("[NETLIFY_FUNCTION_BREVO] BREVO_API_KEY not configured");
      return {
        statusCode: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    console.log("[NETLIFY_FUNCTION_BREVO] Calling Brevo API");
    ...
    console.log("[NETLIFY_FUNCTION_BREVO] Brevo response status:", brevoResponse.status);

    if (!brevoResponse.ok) {
      console.error("[NETLIFY_FUNCTION_BREVO] Brevo API error:", responseData);
      return {
        statusCode: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    console.log("[NETLIFY_FUNCTION_BREVO] Email sent successfully, ID:", responseData.messageId);
    ...
    console.error("[NETLIFY_FUNCTION_BREVO] Exception caught:", error);
