...
serve(async (req)=>{
  try {
    ...
    // Check schedule
    let { data: schedule, error: scheduleError } = await supabase.from('admin_pool_schedule').select('*').eq('year_month', yearMonth).single();
    if (scheduleError && scheduleError.code === 'PGRST116') {
      const randomDates = generateRandomDates();
      console.log(`[${yearMonth}] Creating schedule:`, randomDates);
      const { data: newSchedule, error: insertError } = await supabase.from('admin_pool_schedule').insert({
    ...
    const { data: existingPool } = await supabase.from('nomor_surat').select('*').like('tanggal', `${yearMonth}%`).eq('keterangan', 'ADMIN_EMERGENCY_POOL');
    ...
    const { data: activeCombos } = await supabase.from('nomor_surat').select('kode_kanwil, kode_upt, kode_masalah, kode_submasalah1, kode_submasalah2, nomor_urut').like('tanggal', `${yearMonth}%`).eq('status', 'confirmed').neq('keterangan', 'ADMIN_EMERGENCY_POOL').order('nomor_urut', {
      ascending: false
    });
    ...
    if (!activeCombos || activeCombos.length === 0) {
      ...
      const { data: allCombos } = await supabase.from('nomor_surat').select('kode_kanwil, kode_upt, kode_masalah, kode_submasalah1, kode_submasalah2, nomor_urut').like('tanggal', `${yearMonth}%`).neq('keterangan', 'ADMIN_EMERGENCY_POOL').order('nomor_urut', {
        ascending: false
      });
    ...
    console.log(`[PROCESSING] Need to reserve ${needToReserve} more number(s) to complete pool (current: ${poolCount}/3)`);
    // ======================================
    ...
    // Continue anyway (fail-safe)
      ...
      if (diffMinutes > 5) {
        ...
        // Force release lock
        await supabase.from('reserve_lock').update({
          is_locked: false,
    ...
    for (const combo of Object.values(combinations)){
      ...
      const { data: cancelledBelow } = await supabase.from('nomor_surat').select('nomor_urut, id').eq('kode_kanwil', combo.kode_kanwil).eq('kode_upt', combo.kode_upt).eq('kode_masalah', combo.kode_masalah).eq('kode_submasalah1', combo.kode_submasalah1).eq('kode_submasalah2', combo.kode_submasalah2).eq('status', 'cancelled').neq('keterangan', 'ADMIN_EMERGENCY_POOL').lt('nomor_urut', combo.highest).order('nomor_urut', {
        ascending: false
      }).limit(1);
      let reservedNomor;
      let reserveMethod = '';
      ...
      const nomorLengkap = cleanSubMasalah2 !== "" ? `${combo.kode_kanwil}.${combo.kode_upt}-${combo.kode_masalah}.${combo.kode_submasalah1}.${cleanSubMasalah2}-${reservedNomor}` : `${combo.kode_kanwil}.${combo.kode_upt}-${combo.kode_masalah}.${combo.kode_submasalah1}-${reservedNomor}`;
      const { data, error: insertError } = await supabase.from('nomor_surat').insert({
